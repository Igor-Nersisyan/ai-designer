...
### Улучшение анализа с JSON-форматом
...
### UX улучшения и упрощение workflow
1. **Исправление закрытия проекта**: Добавлены session_state keys для стилей и цветов, чтобы предотвратить сброс проекта при взаимодействии с виджетами
2. **Добавлен стиль "Нейтральный"**: Расширен список стилей для большей гибкости дизайна
3. **Упрощенный выбор варианта**: При клике "Выбрать этот дизайн" все невыбранные варианты автоматически удаляются из базы и session_state
4. **Автоматическая генерация контента**: После выбора варианта автоматически генерируются:
   - Детальные рекомендации по материалам
   - Список покупок
   - Калькуляция бюджета (если было ранее реализовано)
5. **Улучшенная секция рекомендаций**: 
   - Убрано дублирующее изображение выбранного варианта
   - Добавлен подзаголовок "💡 Детальные рекомендации по материалам"
   - Кнопка изменена на "Обновить рекомендации" (вместо "Получить")

### PDF улучшения
1. **Исправлены пропорции изображений**: Добавлен правильный расчет aspect ratio для предотвращения чёрных краёв
2. **Компактная первая страница**: Уменьшены spacer'ы между элементами (с 0.5см до 0.2см) для размещения всего контента на первой странице
3. **Оптимизированный размер изображений**: Максимальный размер уменьшен с 14см до 12см для лучшего размещения на странице

**Технические детали:**
- `needs_generation` флаг в session_state для отслеживания необходимости автогенерации
- Удаление вариантов через SQLAlchemy: `db.query(DesignVariant).filter(...).delete()`
- Aspect ratio расчёт: `aspect = width / height`, затем пропорциональное масштабирование
